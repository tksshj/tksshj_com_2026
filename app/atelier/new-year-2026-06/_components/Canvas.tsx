...
export default function Canvas({ w, h, nPoints }: { w: number; h: number; nPoints: number }) {
  ...
  useEffect(() => {
    ...
    const indices = shuffleIndices(targetPoints.length)
    ...
    const pointsMaterial = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          gl_FragColor = vec4(0.2627, 0.3098, 0.4000, shape);
        }
      `,
    })
    ...
    const tween = new TWEEN.Tween(state)
      ...
      .onUpdate(() => {
        pointsMaterial.uniforms.uTime.value = rafNow + 10000
        pointsMaterial.uniforms.uPos.value = direction == 1 ? 1.0 - state.value : state.value
